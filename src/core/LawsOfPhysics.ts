...
export const LawsOfPhysics = {
  ...
  calculateGibbsFreeEnergy(electronsTransferred: number, standardPotential: number): number {
    return -(electronsTransferred * PhysicalConstants.FARADAY * standardPotential) / 1000;
  }
  ...
  calculateIdealGasVolume(moles: number, temperatureK: number, pressureAtm: number = 1.0): number {
    return (moles * PhysicalConstants.GAS_CONSTANT_L_ATM * temperatureK) / pressureAtm;
  }
  ...
  calculateRadiativePower(temperatureK: number, surfaceArea: number, emissivity: number = 0.9): number {
    return emissivity * PhysicalConstants.STEFAN_BOLTZMANN * surfaceArea * Math.pow(temperatureK, 4);
  }
  ...
  calculatePeakWavelength(temperatureK: number): number {
    return (PhysicalConstants.WIEN_DISPLACEMENT / temperatureK) * 1e9;
  }
  ...
  celsiusToKelvin(temperatureC: number): number {
    return temperatureC + PhysicalConstants.ABSOLUTE_ZERO_OFFSET;
  },

  /** Kelvin to Celsius. */
  kelvinToCelsius(temperatureK: number): number {
    return temperatureK - PhysicalConstants.ABSOLUTE_ZERO_OFFSET;
  },
} as const;
